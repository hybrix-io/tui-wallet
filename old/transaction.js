...
function performTransaction (properties, asset, amount, sourceAddress, targetAddress, base, element, balance, balorig) {
  return function () {
    ...
    if (typeof assets.fact[asset] !== 'undefined') {
      ...
      var unspent = 'a/' + asset + '/unspent/' + sourceAddress + '/' + String(toInt(amount).plus(toInt(fee))) + '/' + targetAddress + (typeof assets.keys[asset].publicKey === 'undefined' ? '' : '/' + assets.keys[asset].publicKey);
      hybrixdcall({r: unspent, z: 1, pass: properties}, 0, function (object, passdata) {
                  ...
                  // DEBUG: logger(JSON.stringify(assets));
                   var onTransaction = function(transaction){
                    if (typeof transaction !== 'undefined') {
                      ...
                      hybrixdcall({r: 'a/' + asset + '/push/' + transaction, z: 1, pass: this.p}, null, function (object, passdata) {
                        var p = passdata;
                        if (typeof object.data !== 'undefined' && object.error === 0) {
                          ...
                          renderMessageAndMaybeRetry(
                            element,
                            'The transaction could not be sent by the hybrixd node! Please try again.',
                            () => { UItransform.txStop(); UItransform.setBalance(element, balorig); logger('Error sending transaction: ' + object.data); },
                            'The transaction could not be sent by the hybrixd node! Retrying in a moment.\n',
