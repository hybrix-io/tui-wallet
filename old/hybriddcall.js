...
initAsset = function(entry,fullmode,callback) {
  if(typeof entry !== 'undefined' && typeof fullmode !== 'undefined') {
    function finalize(dcode,submode) {
      ...
      hybrixdcall({r:'a/'+entry+'/factor',c:GL.usercrypto,s:loop_step,z:0},0, function(object) { if(typeof object.data!='undefined') { assets.fact[entry]=object.data; } });
      var loop_step = next_step();
      hybrixdcall({r:'a/'+entry+'/fee',c:GL.usercrypto,s:loop_step,z:0},0, function(object) { if(typeof object.data!='undefined') { assets.fees[entry]=object.data; } });
      var loop_step = next_step();
      hybrixdcall({r:'a/'+entry+'/contract',c:GL.usercrypto,s:loop_step,z:0},0, function(object) { if(typeof object.data!='undefined') { assets.cntr[entry]=object.data; } });
    }
    ...
    if(typeof assets.modehashes[mode]!='undefined') {
      storage.Get(assets.modehashes[mode]+'-LOCAL', function(dcode) {
        ...
        if(!dcode) { // || typeof assets.mode[entry]=='undefined') {
          hybrixdcall({r:'s/deterministic/code/'+mode,z:0},null,
                      function(object) {
...
// hybrixdcall makes direct calls to hybrixd, waits for the process,
...
// 		- the function postfunction runs after a successful call to hybrixd, while waitfunction runs regularly while the hybrixd process is completing
...
hybrixdcall = function(properties,element,postfunction,waitfunction) {
  if(typeof GL.usercrypto !== 'undefined' && GL.usercrypto) {
    ...
    najax({url: urlrequest, timeout: 30000,
           success: function(encobject){
             ...
             if(object === false) {	// quick and dirty retry function for bad connections (TODO! OPTIMIZE!)
               ...
               // DOESN'T WORK: hybrixdcall(properties,element,postfunction,waitfunction);
             } else {
               ...
               if( typeof UI.text[element]!=='undefined' && UI.text[element].getText()==='?' ) {
                 spinnerStart(element);
               }
               hybrixdproc(element,object,this.vars.postfunction,this.vars.waitfunction,0);
             }
           }.bind({vars:varsmain}), error: function(object){
             spinnerStop(element);
    ...
    function hybrixdproc(element,procobj,postfunction,waitfunction,cnt) {
      ...
      if(typeof procobj.data !== 'undefined') {
        ...
        najax({url: urlrequest, timeout: 30000,
               success: function(result){
                 ...
                 if(typeof object === 'undefined' || object === null) { object = { stopped:null,progress:0 }; }
                 ...
                 if(object.progress < 1 && object.stopped == null) {
                   ...
                   setTimeout( function(element,procobj,postfunction,waitfunction,cnt) { hybrixdproc(element,procobj,postfunction,waitfunction,cnt); } ,(cnt*3000), element,procobj,postfunction,waitfunction,cnt);
