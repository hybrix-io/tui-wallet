                ...
                function(object){
                  ...
                  for(var entry in activeAssetsObj) {
                    ...
                    setTimeout(function(entry) {
                      initAsset(entry,GL.assetmodes[entry], doConsoleInputStuff);
                    },(100*i)+defer,entry);
...
function doAutomaticTransactionIfAvailable () {
  ...
  if (sortedAddrKeys && everyAddrIsAvailable) {
    ...
    if (amountIsNumber) {
      ...
      process.stdout.write('Transaction failed: Amount is not a number.\n');
    }
    return 0;
  }
}

function addNewAssetIfAvailable (assetName) {
  var sortedAddrKeys = Object.keys(assets.addr);
  var everyAddrIsAvailable = GL.assetsActive.every((asset) => sortedAddrKeys.includes(asset));

  if (sortedAddrKeys && everyAddrIsAvailable) {
    var availableAssetNames = Object.keys(GL.assetmodes);
    var assetNameExists = availableAssetNames.includes(assetName)
...
function addNewAssetToWallet (assetName) {
...
function doConsoleInputStuff () {
  var assetName = newAsset; // *GLOBAL*
  var transactionDetails_ = transactionDetails // *GLOBAL*
  var assetNameIsNotNull = assetName !== null;
  var transactionDetailsExist = Object.keys(transactionDetails_).length !== 0;

  if (transactionDetailsExist) {
    doAutomaticTransactionIfAvailable()
  } else if (assetNameIsNotNull) {
    addNewAssetIfAvailable(assetName)
  }
}
