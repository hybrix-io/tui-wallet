          ...
          if((balance.lasttx[j]+60000)<(new Date).getTime()) {
            hybriddcall({r:'a/'+balance.asset[j]+'/balance/'+assets.addr[balance.asset[j]],z:0},element,
                        function(object){
                          ...
                          var entry = balance.asset[j]
                          var symbolName = entry.slice(entry.indexOf('.') + 1);
                          var dollars = renderDollarPrice(symbolName, object.data)
                          UI.text['AssetsD-'+balance.asset[j]].setContent(dollars);
                          return object;
                        }
                       );
          }
...
function mainView() {
  ...
  function assetDisplay() {
    if(GL.assetsActive == null || typeof GL.assetsActive !== 'object') {
      ...
      storage.Set(userStorageKey('ff00-0033'), userEncode(GL.assetsActive));
    }
    ...
    if(missingStarredAssetsIDs !== []){
      ...
      storage.Set(userStorageKey('ff00-0034'), userEncode(newStarredAssets));
    }
