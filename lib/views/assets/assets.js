...
function orderKeys (obj) {
  /* c TODO onst keys = Object.keys(obj).sort((k1, k2) => k1 - k2);
...
function loadSavedAssets (data, dataCallback, errorCallback, progressCallback) {
  hybrix.load({key: global.legacyFilePrefix.assetPreference, legacy: true},
    assetPreferences => initializeSavedAssets(assetPreferences, dataCallback, errorCallback),
    error => {
      global.logger(['info'], 'No stored assets found, loading default.');
      initializeDefaultAssets(error, dataCallback, errorCallback); // No saved asset file found
    }
  );
}
  ...
  UI.edit['ManageAssets'].on('press', function () {
    ...
    UI.edit.Close.on('press', function () {
      ...
      const saveAssets = () => {
        ...
        return hybrix.save({key: global.legacyFilePrefix.assetPreference, assetPreferences, legacy: true},
          refreshView,
          e => {
