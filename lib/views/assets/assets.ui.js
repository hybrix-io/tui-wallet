...
exports.display = function (assets) {
  ...
  for (let symbol in assets) {
    ...
    UI.edit['Starred' + i].on('press', function () {
      const index = this.i;
      const isStarred = this.asset.symbol.isStarred;

      this.asset.symbol.isStarred = !isStarred;

      const assetPreferences = {};
      for (let symbol in global.assets) assetPreferences[symbol] = global.assets[symbol].starred ? 1 : 0;
      hybrix.save({key: global.legacyFilePrefix.assetPreference, assetPreferences, legacy: true}, () => {}, () => {});
