...
exports.display = function (assets) {
  ...
  let i = 0;
  let y;
  global.logger([], JSON.stringify(assets));
  for (let symbol in assets) {
    ...
    const isStarred = asset.starred;
    const starStyle = {
      symbol: isStarred ? '\u2605' : '-',
      fg: isStarred ? 'yellow' : 'white',
      bg: 'black',
      hover: { bg: 'grey' },
      focus: { bg: 'grey' }
    };
    ...
    UI.edit['Starred' + i].on('press', function () {
      const index = this.i;
      const isStarred = this.asset.symbol.starred;
      this.asset[symbol].starred = !isStarred;
      ...
      global.logger([], 'assets.ui::star ' + value);

      hybrix.save({key: global.legacyFilePrefix.assetPreference, value, legacy: true}, () => {}, () => {});

      UI.edit['Starred' + index].content = isStarred ? '\u2605' : '-';
      UI.edit['Starred' + index].style.fg = isStarred ? 'yellow' : 'white';
      screen.render();
