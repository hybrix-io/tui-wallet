  ...
  najax({
    ...
    .done(function (data) {
      ...
      if (clean(data.nonce1).length === 48) {
        ...
        var nonce1Hex = clean(data.nonce1);
        var nonce1 = nacl.from_hex(nonce1Hex);
        var nonce1Hex = nonce1Hex.replace(/^[8-9a-f]/, function (match) { var range = ['8', '9', 'a', 'b', 'c', 'd', 'e', 'f']; return range.indexOf(match); });
        var secretsJSON = { 'nonce1': nonce1Hex, 'nonce2': nonce2_hex, 'client_session_pubkey': session_hexkey };
        var sessionSecrets = JSON.stringify(secretsJSON);

        // using signing method
        var crypt_bin = nacl.encode_utf8(sessionSecrets);
	  ...
	  dataType: 'json'
	      ...
	      var key_array = {'nonce': session_nonce,
                'nonce1': nonce1Hex,
                'nonce2': nonce2_hex,
	                       'session_secsign': session_secsign,
                'session_seckey': session_seckey,
	                       'session_pubsign': session_hexsign,
                'session_pubkey': session_hexkey,
	                       'server_pubsign': crypt_vars.server_sign_pubkey,
                'server_pubkey': crypt_vars.server_session_pubkey};
