...
displayAssets = function displayAssets(passdata) {
  ...
  for (var entry in passdata) {
    ...
    // UI.text['AssetsSt-'+balance.asset[i]] = blessed.text({
    //   parent: UI.box,
    //   padding: { left: 1, right: 1 },
    //   left: '25%',
    //   top: y,
    //   tags: true,
    //   name: 'AssetsS-'+balance.asset[i],
    //   content: balance.starred[i] ? 'Star' : 'Moon',
    //   width: '5%',
    //   height: 1,
    //   style: {
    //     fg: '#cccccc',
    ...
    UI.edit['Starred'+i] = UI.buttonfunc.make(lang.buttonStarred, UI.box, 2, y,this.balance.starred[this.i] ? 'Star' : 'Moon', 'yellow');
    UI.edit['Starred'+i].on('press', function() {
      var toggleStarred = !this.balance.starred[this.i];

      // Set globals === 666
      GL.assetsStarred[this.i] = toggleStarred;
      this.balance.starred[this.i] = toggleStarred;
