...
function renderMessageAndMaybeExit (element, balorig, uiMessage, loggerMessage, commandLineMessage, exitStatus) {
  if (element !== null) {
    UItransform.txStop();
    UItransform.setBalance(element, balorig);
    alert(loggerMessage);
    ...
    renderCmdlMessageAndExit(commandLineMessage, exitStatus);
  }
}
...
function renderCmdlMessageAndExit (message, exitStatus) {
  console.log(message);
  fs.writeFileSync('exitStatus.txt', exitStatus);
  setTimeout(() => process.exit(exitStatus), 4000);
}
