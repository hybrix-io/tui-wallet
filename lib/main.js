...
nacl = require('./crypto/nacl'); nacl = nacl.instantiate();
...
var ops = stdio.getopt({
    ...
    'sendtransaction': {key: 'tx', args: 3, description: 'Send transaction [argument: asset] [argument: amount] [argument: target address]'}
  });

if (ops.userid) { userid=ops.userid; };
if (ops.passwd) { passwd=ops.passwd; };
if (ops.newaccount) { PRNG.seeder.mkNewAccount(); };
if (ops.sendtransaction) {
  transactionDetails = {
    asset: ops.sendtransaction[0],
