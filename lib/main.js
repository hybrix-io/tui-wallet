const DEFAULT_HOSTNAME = 'http://127.0.0.1:1111/';
...
const hostname = ops.hostname || DEFAULT_HOSTNAME; // TODO check conf
...
if (!ops.userid) {
  if (!ops.passwd) {
    input.read(lang.viewLoginUser, {capitalize: true}, (e, username) => {
      input.read(lang.viewLoginPass, {password: true}, (e, password) => login(username, password));
    });
  } else input.read(lang.viewLoginUser, {capitalize: true}, (e, username) => login(username, ops.passwd));
} else if (!ops.passwd) input.read(lang.viewLoginPass, {password: true}, (e, password) => login(ops.userid, password));
else login(ops.userid, ops.passwd);
