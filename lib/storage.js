...
var storage = (function() {
  ...
  var SetFile = function(storekey,storevalue) {
    ...
    fs.writeFileSync(storepath+fold+storekey+'.meta',JSON.stringify({time:Date.now(),hash:DJB2.hash(JSON.stringify(storevalue))}));
  }
            ...
            hybriddcall({r:'s/storage/get/'+storekey,c:GL.usercrypto,s:loop_step,z:0},0, function(object) {
              if(typeof object.data==='undefined' || object.data===null || object.data==='null') {
                ...
                try {
                  ...
                  SetFile(storekey+'.meta',JSON.stringify({time:Date.now(),hash:DJB2.hash(JSON.stringify(object.data))}));
                  return true;
                } catch(e) {
                  return false;
                }
