...
function retrievePrice (symbol, balance) {
  hybrix.rout({query: '/engine/valuations/rate/' + symbol + '/usd/' + balance},
    price => {
      UI.text['AssetsD-' + symbol].setContent('' + Math.floor(price * 100) / 100);
    },
    e => {
      UI.text['AssetsD-' + symbol].setContent('N/A');
    }
  );
}
...
exports.activate = function (userID) {
  ...
  UI.topbar = blessed.box({
    ...
    width: '100%+1',
    height: 1,
    content: ' {bold}Internet of Coins{/bold} - Powered by hybrix {|}{bold}{white-fg}' + userID + '{/white-fg}{/bold}  ', // TODO user id weergeven
    tags: true,
    draggable: false,
    border: false,
  ...
  UI.scrollWindow = function (window, y) {
    let scroll = window.getScroll();
    let height = window.getScrollHeight();
    if (y + 1 > scroll) {
